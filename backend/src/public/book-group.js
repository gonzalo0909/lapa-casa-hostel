...
function buildLinkCard(l, guestIdx) {
  var waHref = l.waUrl || ('https://wa.me/?text=' + encodeURIComponent(l.url));
  return '<div class="link-card">' +
    '<div class="link-num">Invitado ' + guestIdx + '</div>' +
    '<div class="link-url">' + l.url + '</div>' +
    '<div class="link-actions">' +
      '<a class="btn-wa" href="' + waHref + '" target="_blank" rel="noopener">' +
        WA_SVG + ' Compartir' +
      '</a>' +
      '<button class="btn-copy" id="copy' + guestIdx + '" onclick="copyLink(' + guestIdx + ', \'' + l.url + '\')">Copiar</button>' +
    '</div>' +
  '</div>';
}
...
function renderLinks(data, ci, co, nights) {
  ...
  // Primer link = cama del TITULAR (la completa él/ella primero)
  if (links.length > 0) {
    var titLink = links[0];
    document.getElementById('titularCard').style.display = 'block';
    document.getElementById('titularUrl').textContent    = titLink.url;
    document.getElementById('btnGoReserve').href         = titLink.url;
  }

  // Resto = invitados
  var guestLinks = links.slice(1);
  if (guestLinks.length > 0) {
    document.getElementById('guestsLbl').style.display = 'block';
  }
  var container = document.getElementById('linksPanel');
  container.innerHTML = guestLinks.map(function(l, idx) {
