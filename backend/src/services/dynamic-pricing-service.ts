...
class DynamicPricingService {
  ...
  async getUnitConfigs(): Promise<UnitConfig[]> {
    const res = await query<UnitConfig>(
      `SELECT
         uc.id, uc.room_type_id, rt.name AS room_name, rt.property_type,
  ...
  async upsertUnitConfig(
    roomTypeId: string,
    data: { min_price_brl?: number | null; max_price_brl?: number | null; bot_enabled?: boolean; notes?: string }
  ): Promise<UnitConfig> {
    await query(
      `INSERT INTO dynamic_pricing_unit_config (room_type_id, min_price_brl, max_price_brl, bot_enabled, notes, updated_at)
       ...
       ON CONFLICT (room_type_id) DO UPDATE SET
         ...
         updated_at    = now()`,
      [roomTypeId, data.min_price_brl ?? null, data.max_price_brl ?? null,
       data.bot_enabled ?? true, data.notes ?? null]
    );
  ...
  async getCalendar(days = 90, roomTypeId?: string): Promise<PriceCacheEntry[]> {
    ...
    let filter = '';
    if (roomTypeId) { args.push(roomTypeId); filter = `AND pc.room_type_id = $${args.length}`; }
  ...
  private eventFactor(
    date: Date, events: PricingEvent[], propertyType: 'hostel' | 'apartment'
    ...
    for (const unit of activeUnits) {
      for (let d = 0; d < cfg.horizon_days; d++) {
        ...
        try {
          ...
          const { factor: eFactor, name: eName } = this.eventFactor(date, events, unit.property_type);
          ...
          await query(
            `INSERT INTO price_cache
             ...
             DO UPDATE SET
               ...
               calculated_at = now()`,
            [dateStr, unit.room_type_id, unit.property_type, base, final,
             oFactor, pFactor, dFactor, eFactor, eName ?? null, occPct]
          );
          processed++;
        } catch (err) {
  ...
  async getRioEvents(): Promise<{ name: string; date: string; venue?: string; url?: string }[]> {
    try {
      ...
      return (data?.data ?? []).slice(0, 20).map((e: any) => ({
        ...
        venue: e.address?.name ?? '', url: e.url ?? '',
      }));
    } catch {
      return [
