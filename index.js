...
const ADMIN_TOKEN = process.env.ADMIN_TOKEN || "";         // legado (permanece)
const ADMIN_USER = process.env.ADMIN_USER || "admin";
const ADMIN_PASS = process.env.ADMIN_PASS || ADMIN_TOKEN;
const ADMIN_SESSION_SECRET = process.env.ADMIN_SESSION_SECRET || ADMIN_TOKEN || "change-me";
...
app.get(["/crons/holds-sweep","/api/crons/holds-sweep"], async (req,res)=>{
  try{
    if (!CRON_TOKEN || (req.query.token||"") !== CRON_TOKEN) return res.status(401).json({ ok:false, error:"unauthorized" });
    const now = Date.now();
    let released = 0;
    for (const [holdId, info] of [...holdsMem.entries()]) {
      if (info.expiresAt && info.expiresAt <= now) {
        try { await postToSheets({ action:"upsert_booking", booking_id: holdId, pay_status:"released" }); }
        catch(e){ logPush("holds_sweep_warn",{ holdId, msg:e?.message||String(e) }); }
        holdsMem.delete(holdId); released++;
      }
    }
...
/* ========================
   Admin login/sesión (token legado + cookie simple)
   ======================== */
