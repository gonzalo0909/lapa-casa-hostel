"use strict";
/**
 * services/payments-mp.js — Mercado Pago (SDK v2)
 */
...
function ensure(){ if(!client) throw new Error("mp_not_configured"); }
...
async function createPreference(order = {}, { baseUrl = "" } = {}) {
  ...
  const r = await pref.create({
    body: {
      items: [{ title:"Lapa Casa Hostel — Reserva", quantity:1, unit_price: amountBRL, currency_id:"BRL" }],
      metadata: { booking_id },
...
function buildMpWebhookHandler({ notifySheets, isDuplicate=()=>false, log=()=>{} } = {}) {
  ...
  return async (req, res) => {
    try {
      ...
      if (!type || !idRaw) return res.status(200).json({ ok:true, ignored:true });

      if (type === "payment") {
        const key = `mp:payment:${idRaw}`;
        if (isDuplicate && isDuplicate(key)) return res.status(200).json({ ok:true, dedup:true });

        let p; try { p = await payment.get({ id: idRaw }); } catch (e) { log("mp_payment_get_error", { idRaw, err:String(e?.message||e) }); }
      ...
      if (type === "merchant_order") {
        ...
        let mo; try { mo = await morder.get({ merchantOrderId: idRaw }); } catch (e) { log("mp_mo_get_error", { idRaw, err:String(e?.message||e) }); }
        ...
        if (Array.isArray(mo?.payments)) {
          for (const pay of mo.payments) {
            ...
            if (st === "approved") approvedSum += Number(pay.total_paid_amount || pay.transaction_amount || 0);
          }
        }
...
async function safeNotify(fn, bookingId, status, amount, log) {
  try { await fn({ action:"payment_update", booking_id: bookingId, total: amount, status }); }
