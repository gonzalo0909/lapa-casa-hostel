(function(){
  // Configuración desde las variables globales
  const API_BASE = (window.BACKEND_BASE_URL || "/api").replace(/\/$/, "");
  ...
  const PAY_STATUS_ENDPOINT     = endpoint("/payments/status");

  const STRIPE_PK = window.STRIPE_PUBLISHABLE_KEY || "pk_test_51Ru1ZgGcpzl4zlUFGhibCAqJMKWCse2m2auKYWQkGxBIQ9ZtoGvPwoyrjwzmZYMEq1mbL4XZEo3TP7cuhhUHgtg000iI4zzsRh";
  ...
  const I18N={ es:{
    ...
    test_title:"Test mode?", test_sub:"“Simulate approved payment” allows Confirm without MP/Stripe.", test_btn:"Simulate approved payment",
  ...
  continueBtn?.addEventListener('click',async ()=>{
    ...
    try{
      ...
      requestAnimationFrame(()=>window.scrollTo({ top: formCard.offsetTop-10, behavior:'smooth' }));
  ...
  simulateOkBtn?.addEventListener('click',()=>{
    payStateEl.textContent=I18N[LANG].approved;
  ...
  payMPBtn?.addEventListener('click', async ()=>{
    ...
    finally{ mpBusy=false; payMPBtn.disabled=false; }
  });
  ...
  payStripeBtn?.addEventListener('click', async ()=>{
    ...
    finally{ stripeBusy=false; payStripeBtn.disabled=false; }
  });
  ...
  document.getElementById('reserva-form')?.addEventListener('submit', async e=>{
    ...
    try{
      if(!$('#consentChk').checked){ alert(I18N[LANG].need_consent); submitBtn.disabled=false; return; }
      const order = buildOrderBase();
      let paidOk = (payStateEl.textContent||'').toLowerCase().includes(I18N[LANG].approved.slice(0,5)) || window.__simApproved;
      try{
  ...
  // Restaura estado pagado al volver
    ...
    try{
      ...
      if(cached && cached.bookingId && PAY_STATUS_ENDPOINT){
        fetch(`${PAY_STATUS_ENDPOINT}?bookingId=${encodeURIComponent(cached.bookingId)}`)
          ...
          .then(js=>{ if(js && (js.paid===true || js.status==='approved'||js.status==='paid')){ payStateEl.textContent=I18N[LANG].approved; submitBtn.disabled=false; window.__simApproved=true; }})
          .catch(()=>{});
      }
    }catch(_){}
  })();
