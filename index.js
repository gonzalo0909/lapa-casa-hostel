"use strict";
/**
 * Lapa Casa Backend — Render (clean start)
 * Rutas: / (static), /api/health, /availability, /bookings, /holds/*, /events,
 *        /payments/stripe/session, /webhooks/stripe, /payments/mp/preference, /webhooks/mp,
 *        /crons/holds-sweep?token
 */
...
app.post(["/webhooks/stripe","/api/webhooks/stripe"], express.raw({ type:"application/json" }), async (req,res)=>{
  try{
    ...
    catch(err){ logPush("stripe_error",{ where:"invalid_signature", msg: err?.message||String(err) }); return res.status(400).send("invalid signature"); }
...
/* ===== Middlewares luego del webhook Stripe ===== */
const corsOptions = {
  origin: (origin, cb) => {
...
/* ===== Payments: Mercado Pago preference + webhook ===== */
...
app.post(["/webhooks/mp","/api/webhooks/mp"], async (req,res)=>{
  try{
    ...
    if (MP_WEBHOOK_SECRET) {
      ...
      if (!ok) { logPush("mp_signature_fail",{ paymentId }); return res.status(401).send("invalid signature"); }
    }
...
app.post(["/bookings","/api/bookings"], rateLimit(60), async (req,res)=>{
  try{
    ...
    if (!j?.ok) { const fb = { ...payload }; delete fb.action; j = await postToSheets(fb); }
    invalidateAvailabilityCache();
...
app.get(["/crons/holds-sweep","/api/crons/holds-sweep"], (req,res)=>{
  ...
  invalidateAvailabilityCache();
  res.json({ ok:true, pruned, holds: holdsMem.size });
});

/* ===== Sheets helpers / Availability ===== */
...
function calcOccupiedBeds_(rows, from, to) {
  ...
  for (const row of rows) {
    ...
    try { if (typeof cjson === "string") cjson = cjson ? JSON.parse(cjson)
