class FormValidator {
    ...
    this.fields = {
      nombre: {
        validators: [
          ...
          { fn: v => /^[a-zA-ZáéíóúÁÉÍÓÚñÑ àèìòùÀÈÌÒÙâêîôûÂÊÎÔÛãõÃÕçÇ\s]+$/.test(v?.trim() || ''), msg: 'Solo letras y espacios' },
  ...
  filterInput(fieldId, event) {
    ...
    switch (fieldId) {
      case 'nombre':
        value = value.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑ àèìòùÀÈÌÒÙâêîôûÂÊÎÔÛãõÃÕçÇ\s]/g, '');
        break;
      case 'telefono':
