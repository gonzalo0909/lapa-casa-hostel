...
router.post("/stripe/session", async (req,res)=>{
  try{
    ...
    if (typeof order.total === "undefined") return res.status(400).json({ ok:false, error:"missing_total" });
    const out = await createCheckoutSession(order, { baseUrl: baseUrl(req) });
...
router.post("/mp/preference", async (req,res)=>{
  try{
    const order = req.body?.order || req.body || {};
    if (typeof order.total === "undefined") return res.status(400).json({ ok:false, error:"missing_total" });
    const out = await createPreference(order, { baseUrl: baseUrl(req) });
    res.json({ ok:true, ...out });
  }catch(e){ res.status(500).json({ ok:false, error:String(e.message||e) }); }
});
