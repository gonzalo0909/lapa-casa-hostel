(function(){
  ...
  (async ()=>{ try{ await fetchJSON(EP.PING); }catch(e){ alert('⚠️ API no disponible. Detalle: '+String(e.message||e)); } })();
  ...
  function toggleBed(roomId,bedId,dateIn,dateOut,btn){
    ...
    set.has(bedId)? set.delete(bId): set.add(bId);
    selection[roomId]=set; btn.classList.toggle('selected', set.has(bId));
