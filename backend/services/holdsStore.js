...
function listHolds() { sweepExpired(); return Array.from(holds.values()).sort((a,b)=> (a.createdAt>b.createdAt?-1:1)); }
function getHold(holdId) { sweepExpired(); return holds.get(String(holdId)); }
...
function releaseHold(holdId) { sweepExpired(); holds.delete(String(holdId)); return { ok: true }; }
...
function getHoldsMap(fromYMD, toYMD) {
  ...
  for (const h of holds.values()) {
    ...
    const overlaps = !f && !t ? true : Boolean((!t || (hin && hin < t)) && (!f || (hout && hout > f)));
    if (!overlaps) continue;
    for (const [rid, beds] of Object.entries(h.camas || {})) (beds||[]).forEach(b => out[Number(rid)]?.add(Number(b)));
  }
  const result = {}; for (const k of [1,3,5,6]) result[k] = Array.from(out[k]||[]).sort((a,b)=>a-b);
  return result;
}

module.exports = { listHolds, getHold, startHold, confirmHold, releaseHold, sweepExpired, getHoldsMap };
