...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Verificación mínima (si falla, nada funciona)
  const requiredEls = { dateIn, dateOut, men, women, checkAvail, roomsCard, continueBtn };
  for (const [k, v] of Object.entries(requiredEls)) {
    if (!v) {
      console.error(`[INIT] Falta elemento #${k}. Revisa IDs en index.html`);
      // no corto la ejecución, pero aviso
    }
  }
  ...
  function setupGuestControls() {
    function updateNeeded() {
      ...
      if (continueBtn) continueBtn.disabled = needed > 0 && selectedBeds !== needed;
    }
  ...
  function renderRooms(occupied, hombres, mujeres) {
    ...
    toShow.forEach(roomId => {
      ...
      for (let i = 1; i <= totalBeds; i++) {
        ...
        if (taken.has(i)) bedEl.classList.add("occupied", "disabled");
    ...
    if (roomsCard) roomsCard.style.display = toShow.length ? "block" : "none";
  }
  ...
  checkAvail?.addEventListener("click", async () => {
    ...
    try {
      ...
      renderRooms({}, hombres, mujeres);
    }

    if (selCountEl) selCountEl.textContent = 0;
    if (neededEl) neededEl.textContent = need;
  ...
  document.getElementById("rooms")?.addEventListener("click", (e) => {
    ...
    const needed = (parseInt(men?.value || 0) + parseInt(women?.value || 0));
    if (continueBtn) continueBtn.disabled = count !== needed;
  });

  continueBtn?.addEventListener("click", async () => {
    const order = buildOrderBase();
    if (!order) return alert("Fechas no válidas");
    if (Object.keys(order.camas).length === 0) return alert("Seleccioná al menos una cama.");
    ...
    try {
      ...
      if (!j.ok || !j.holdId) return alert("Error: " + (j.error || "No se creó el hold"));

      if (formCard) formCard.style.display = "block";
      const ps = document.getElementById("payState");
      if (ps) ps.textContent = "pendiente";
    } catch (e) {
  ...
  payStripe?.addEventListener("click", async () => {
    if (!STRIPE_KEY) return alert("Stripe no está configurado correctamente.");
    try {
      const order = buildOrderBase();
  ...
  // Inicializar
  setupGuestControls();
  console.log("[INIT] main.js listo");
});
