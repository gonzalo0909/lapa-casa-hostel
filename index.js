// /index.js  (archivo único)
// Reemplaza TODO tu index.js con este contenido.
...
// ====== ENV EVENTOS (agregador ICS/RSS/JSON)
...
const EVENTS_TTL_MS = Math.max(1, EVENTS_TTL_HOURS) * 60 * 60 * 1000;
...
app.post("/webhooks/stripe", express.raw({ type:"application/json" }), async (req,res)=>{
  try{
    ...
    catch (err) { logPush("stripe_bad_sig", { msg: err.message }); return res.status(400).send("invalid signature"); }
    ...
    async function notifySheets(bookingId, status, total) {
      ...
      } catch (e) { logPush("sheets_error", { via:"stripe", msg:e?.message||String(e) }); }
    }
    ...
    if (t === "checkout.session.completed") {
      ...
      let bookingId = "";
      if (c.payment_intent) {
        try { const piObj = await stripe.paymentIntents.retrieve(c.payment_intent); bookingId = piObj?.metadata?.bookingId || ""; } catch {}
      }
  ...
  } catch(e){ logPush("stripe_webhook_error", { msg: e?.message||String(e) }); return res.status(200).send("ok"); }
});
...
app.post("/payments/stripe/session", rateLimit(30), async (req,res)=>{
  ...
  }catch(err){ logPush("stripe_session_error", { msg: err?.message||String(err) }); res.status(500).json({ error:"stripe_session_error" }); }
});
...
app.post("/payments/mp/preference", rateLimit(30), async (req,res)=>{
  ...
  }catch(err){ logPush("mp_pref_error", { msg: err?.message||String(err) }); res.status(500).json({ error:"mp_preference_failed" }); }
});
...
app.post("/webhooks/mp", async (req,res)=>{
  try{
    ...
    return res.status(200).send("ok");
  } catch(e){ logPush("mp_webhook_error",{ msg:e?.message||String(e) }); return res.status(200).send("ok"); }
});
...
app.post("/bookings", rateLimit(60), async (req,res)=>{
  ...
  }catch(e){ logPush("bookings_error",{ msg:e?.message||String(e) }); res.status(500).json({ ok:false, error:"forward_failed" }); }
});
...
app.get("/availability", async (req,res)=>{
  try{
    ...
    res.json(out);
  }catch(e){ logPush("availability_error", { msg: e?.message||String(e) }); res.status(500).json({ ok:false, error:"availability_failed" }); }
});
...
app.get("/ical/:roomId.ics", async (req,res)=>{
  try{
    ...
    res.send(ics);
  }catch(e){ logPush("ical_export_error",{ msg:e?.message||String(e) }); res.status(500).send("ERROR"); }
});
...
app.get("/crons/ical-pull", async (req,res)=>{
  ...
  }catch(e){ logPush("ical_import_error",{ msg:e?.message||String(e) }); res.status(500).json({ ok:false, error:"ical_pull_failed" }); }
});

// ====== Eventos (agregador ICS/RSS/JSON con cache)
let eventsCache = { ts:0, data:[] };

function toISODateOnly_(yyyymmdd) {
...
function toISOAny_(s) {
  if (!s) return null;
  const d = new Date(s);
  if (!isNaN(d.getTime())) return d.toISOString();
  const only = String(s).replace(/\D/g,"");
  if (only.length === 8) return toISODateOnly_(only);
  return null;
}
function textAfterColon_(line){ const i=line.indexOf(":"); return i>=0 ? line.slice(i+1) : ""; }
function unescapeCDATA_(t){ return String(t||"").replace(/<!\[CDATA\[|\]\]>/g,"").trim(); }
function normalizeEv_(ev){
  return {
    title: String(ev.title || "Evento").slice(0,140),
    start: toISOAny_(ev.start) || new Date().toISOString(),
    venue: String(ev.venue || ev.location || "").slice(0,140),
    source: String(ev.source || "")
  };
}
function parseICSFeed_(text, source){
  const lines = String(text||"").split(/\r?\n/);
  const out=[]; let inEv=false, summary="", dtStart="", location="";
  for(const raw of lines){
    const line = raw.trim();
    if(line==="BEGIN:VEVENT"){ inEv=true; summary=""; dtStart=""; location=""; continue; }
    if(line==="END:VEVENT"){
      const startISO = (dtStart.includes("T") ? toISOAny_(dtStart.replace("Z","")) : null) || toISODateOnly_(dtStart.replace(/\D/g,""));
      if(startISO) out.push(normalizeEv_({ title: summary||"Evento", start:startISO, venue:location, source }));
      inEv=false; continue;
    }
    if(!inEv) continue;
    if(line.startsWith("SUMMARY")) summary = textAfterColon_(line);
    else if(line.startsWith("DTSTART")) dtStart = textAfterColon_(line);
    else if(line.startsWith("LOCATION")) location = textAfterColon_(line);
  }
  return out;
}
function parseRSSorAtom_(text, source){
  const xml = String(text||""); const items=[];
  // RSS <item>
  ...
  let m; while((m=rssItemRe.exec(xml))){
    const block=m[0];
    const title=unescapeCDATA_((block.match(rssTitleRe)||[,""])[1]);
    const dateRaw=(block.match(rssDateRe)||[,"",""])[1] || (block.match(rssDateRe)||[,"",""])[2] || "";
    const venue=unescapeCDATA_((block.match(rssVenueRe)||[,""])[1]||"");
    const start=toISOAny_(dateRaw) || new Date().toISOString();
    items.push(normalizeEv_({ title, start, venue, source }));
  }
  // Atom <entry>
  ...
  while((m=entryRe.exec(xml))){
    const block=m[0];
    const title=unescapeCDATA_((block.match(entryTitleRe)||[,""])[1]);
...
function parseJSONFeed_(text, source){
  try{
    const j = JSON.parse(String(text||"null"));
    const arr = Array.isArray(j) ? j : Array.isArray(j?.events) ? j.events : [];
    return arr.map(e=> normalizeEv_({ ...e, source }));
  }catch{ return []; }
}
async function fetchEventsFrom_(url){
  try{
    const res = await fetch(url, { method:"GET" });
    const txt = await res.text();
    if (/BEGIN:VCALENDAR/.test(txt) || /\.ics(\?|$)/i.test(url)) return parseICSFeed_(txt, url);
    if (/<rss\b|<feed\b/i.test(txt) || /\.xml(\?|$)/i.test(url))   return parseRSSorAtom_(txt, url);
    if (/^\s*[\[{]/.test(txt))                                     return parseJSONFeed_(txt, url);
    return [];
  }catch{ return []; }
}
async function loadEvents_(){
  ...
  if (ENABLE_EVENTS && EVENTS_FEEDS.length){
    for (const u of EVENTS_FEEDS){
      const items = await fetchEventsFrom_(u);
...
app.get("/api/events", async (_req,res)=>{
  try{
    const now=Date.now();
    if (now - eventsCache.ts > EVENTS_TTL_MS) {
      eventsCache.data = await loadEvents_();
      eventsCache.ts = now;
    }
    res.json({ ok:true, events: eventsCache.data });
...
app.get("/admin", adminAuth, async (_req,res)=>{
  ...
  <p><a href="/admin/rows.csv">Descargar CSV</a></p>`;
...
// ====== Helpers (Sheets + availability + ICS)
...
function calcOccupiedBeds_(rows, from, to){
  ...
  for(const roomId of Object.keys(ROOMS)){
    ...
    if(need>0){ for(let b=1, added=0; b<=cap && added<need; b++){ if(!set.has(b)){ set.add(b); added++; } } }
  }
  const out = {};
...
function parseICS_(text){
  const lines = String(text||"").split(/\r?\n/);
  const events=[]; let inEvent=false; let dtStart=null, dtEnd=null;
  for(const raw of lines){
    const line = raw.trim();
