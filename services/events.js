// ▶ Render → services/events.js → public/index.html usa GET /api/events
"use strict";

/**
 * Eventos con cache y feeds (ICS/RSS/Atom/JSON)
 * ENV:
 *  - ENABLE_EVENTS=1
 *  - EVENTS_TTL_HOURS=24
 *  - EVENTS_FEEDS="https://foo.ics, https://bar.rss, https://baz.json"
 */
...
const TTL_MS = Math.max(1, EVENTS_TTL_HOURS) * 3600 * 1000;
let cache = { ts: 0, data: [] };
...
function toISOAny(s) {
  ...
  const dateOnly = String(s).replace(/\D/g, "");
...
async function loadAll() {
  ...
  if (ENABLE_EVENTS && EVENTS_FEEDS.length) {
    for (const u of EVENTS_FEEDS) out = out.concat(await fetchOne(u));
  }
  if (!out.length) {
    out = [
